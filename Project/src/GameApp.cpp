...
void GameApp::UpdateScene(float dt)
{
    ...
    // update collision fot visual cone and object
...
void GameApp::DrawScene()
{
    ...
    if (static_cast<uint32_t>(m_LightCullTechnique) >= 2)
    {
        ...
        {
            m_DeferredEffect.DebugNormalGBuffer(m_pd3dImmediateContext.Get(),
                m_pDebugNormalGBuffer->GetRenderTarget(), m_pGBufferSRVs[0], m_pCamera->GetViewPort());
        ...
        {
            m_pd3dImmediateContext->ResolveSubresource(m_pDebugAlbedoGBuffer->GetTexture(), 
                0, m_pGBuffers[1]->GetTexture(), 0, DXGI_FORMAT_R8G8B8A8_UNORM);
            ImVec2 winSize = ImGui::GetWindowSize();
            float smaller = (std::min)((winSize.x - 20) / AspectRatio(), winSize.y - 36);
...
void GameApp::InitLightParams()
{
    ...
    // handle random seed as its uniformity 
...
void GameApp::ResizeBuffers(UINT width, UINT height, UINT msaaSamples)
{
    ...
    // INIT: all resource for deferred rendering 
    ...
    // sample mask for msaa (if msaa enable)
    ...
    // G-Buffer
    // MRT required all G-Buffer use same level of MSAA
    m_pGBuffers.clear();
    // normal_specular
    ...
    // reading depth buffer as Last SRV 
...
{
    // Note: 我们实际上只需要清空深度缓冲区，因为我们用天空盒的采样来替代没有写入的像素(例如：远平面)
...
void GameApp::RenderSkybox()
{
    m_GpuTimer_Skybox.Start();
    {
        ...
        // full screen draw not require for depth 
        // so no need for back buffer clear 
        ID3D11RenderTargetView* pRTVs[] = { GetBackBufferRTV() };
        m_pd3dImmediateContext->OMSetRenderTargets(1, pRTVs, nullptr);
