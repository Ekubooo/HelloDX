...
bool BasicEffect::InitAll(ID3D11Device * device)
{
    ...
    // 创建顶点着色器和顶点布局
    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Triangle_VS.cso", L"HLSL\\GSTest\\Triangle_VS.hlsl", "VS", "vs_5_0", blob.GetAddressOf()));
    ...
    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Cylinder_VS.cso", L"HLSL\\GSTest\\Cylinder_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    ...
    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Normal_VS.cso", L"HLSL\\GSTest\\Normal_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    HR(device->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pNormalVS.GetAddressOf()));

    // 创建像素着色器
    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Triangle_PS.cso", L"HLSL\\GSTest\\Triangle_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
    HR(device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pTrianglePS.GetAddressOf()));

    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Cylinder_PS.cso", L"HLSL\\GSTest\\Cylinder_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
    HR(device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pCylinderPS.GetAddressOf()));

    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Normal_PS.cso", L"HLSL\\GSTest\\Normal_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
    HR(device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pNormalPS.GetAddressOf()));

    // 创建几何着色器
    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Triangle_GS.cso", L"HLSL\\GSTest\\Triangle_GS.hlsl", "GS", "gs_5_0", blob.ReleaseAndGetAddressOf()));
    HR(device->CreateGeometryShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pTriangleGS.GetAddressOf()));

    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Cylinder_GS.cso", L"HLSL\\GSTest\\Cylinder_GS.hlsl", "GS", "gs_5_0", blob.ReleaseAndGetAddressOf()));
    HR(device->CreateGeometryShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pCylinderGS.GetAddressOf()));

    HR(CreateShaderFromFile(L"HLSL\\GSTest\\Normal_GS.cso", L"HLSL\\GSTest\\Normal_GS.hlsl", "GS", "gs_5_0", blob.ReleaseAndGetAddressOf()));
