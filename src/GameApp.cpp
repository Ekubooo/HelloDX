...
void GameApp::UpdateScene(float dt)
{
    ...
    if (ImGui::Begin("Lighting"))
    {
        ...
        ImGui::Text("Light");
        if (ImGui::Combo("Light type", &curr_ligh_item, light_modes, ARRAYSIZE(light_modes)))
        {
            m_PSConstantBuffer.dirLight     = (curr_ligh_item == 0 ? m_DirLight : DirectionalLight());
            m_PSConstantBuffer.pointLight   = (curr_ligh_item == 1 ? m_PointLight : PointLight());
            m_PSConstantBuffer.spotLight    = (curr_ligh_item == 2 ? m_SpotLight : SpotLight());
        }
        bool light_changed = false;
        ImGui::PushID(curr_ligh_item);
        if (curr_ligh_item == 0)
        {
